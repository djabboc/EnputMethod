...
输入如 `he` 的前缀时，编辑区域保留 `he`，悬浮候选窗按页显示匹配词。按 `-` 或 `+` 翻页；使用上下键跨页移动高亮候选。鼠标可选择候选，或使用上一页/下一页控件，且不会抢走编辑器焦点。按当前页对应数字键选择单词，或按 `Tab` 选择高亮单词。已有组合输入或候选联想时，Shift 和 Escape 可取消活动组合输入，`F2` 切换 Emoji 候选，`F3` 显示富文本翻译视图；空闲状态的快捷键会交给当前应用，因此文件资源管理器的 `F2` 重命名、`F5` 刷新和 Escape 取消不受影响。翻译窗口标题栏的 `Copy` 按钮复制完整翻译。
