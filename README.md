...
输入如 `he` 的前缀时，编辑区域保留 `he`，悬浮候选窗按页显示匹配词。按 `-` 或 `+` 翻页；使用上下键跨页移动高亮候选。鼠标可选择候选，或使用上一页/下一页控件，且不会抢走编辑器焦点。按当前页对应数字键选择单词，或按 `Tab` 选择高亮单词。Shift 和 Escape 可取消活动组合输入。`F2` 开启 Emoji 模式，`F3` 显示富文本翻译视图；翻译窗口标题栏的 `Copy` 按钮复制完整翻译。
