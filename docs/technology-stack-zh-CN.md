...
- 翻译窗使用只读 `RichTextBox + FlowDocument`，用不同 Run 渲染标题、语言标签、词性、义项和例句；标题栏的非激活 Copy 按钮复制完整文本，成功后显示短暂的勾选确认；支持滚动、四边/四角缩放和尺寸防抖持久化。
- 候选协议携带完整 composition 屏幕矩形。候选窗优先置于下方，底部不足时放到上方，左右位置始终收束在同一显示器工作区；原生回退窗复用同一规则。
...
候选消息携带候选列表、页码、选中索引、composition 屏幕矩形、主题、模式标记和配置快照。翻译消息携带结构化翻译视图及候选窗边界，Overlay 据此将翻译窗放在候选窗侧方并约束到工作区。
...
发布不要求终端用户安装 Visual Studio 或 .NET SDK。`scripts\publish-release.ps1 -Version <version>` 默认产出完整的 Windows x64 ZIP，但按产品决定不包含 .NET Runtime；系统缺少 .NET 9 Desktop Runtime x64 时由 Windows 提示用户安装。用户在解压根目录运行 WPF `Install Enput Method.exe` 或 `Uninstall Enput Method.exe`。两个启动器各自携带相邻的托管 DLL、deps 和 runtimeconfig，运行时资源位于共同的 `payload`。
