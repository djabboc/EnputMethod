...
传输使用当前用户会话内的命名管道。每个会话状态含递增 `stateId`，避免迟到的鼠标动作提交已经过期的候选。
...
- [x] Overlay 一次只接受一个 Host 管道连接，并可作为后台伴随进程运行；由 Native Host 自动启动留待任务 3。
- [x] 验收：Overlay 可独立运行，接收测试消息并输出动作消息；无 TSF 或 COM 依赖。

## 任务 2：WPF 非激活窗口

- [ ] 实现候选和翻译两个透明、无任务栏、无激活的 WPF 窗口。
- [ ] 通过 `HwndSource`/Win32 扩展样式设置 `WS_EX_NOACTIVATE`、`WS_EX_TOOLWINDOW`。
- [ ] 实现竖排/横排、页脚、模式标记、主题、Emoji、翻译滚动和鼠标命中。
- [ ] 验收：在独立测试宿主中点击候选不抢焦点，窗口位置和尺寸稳定。

当前进度：候选和翻译窗口、`WS_EX_NOACTIVATE`、候选点击及翻页动作回传、基础翻译滚动已实现；主题映射、Emoji 专用绘制、独立测试宿主和人工焦点验收仍待完成。
...
- [ ] 将正常候选显示切换至 Overlay，原生候选窗只用作故障回退。
- [ ] 用 Overlay 鼠标动作覆盖选择、前后翻页和悬停反馈。
- [ ] 验收：Notepad、EmEditor、VS Code、浏览器和 ChatGPT 中的键盘、鼠标、连续联想、Emoji、分页和边缘定位都正确。

## 任务 5：迁移翻译 UI 与发布

- [ ] 将翻译视图、主题字段和滚动条迁入 Overlay。
- [ ] 为 IPC 和视图模型增加离线测试；保留完整 Release|x64 构建。
- [ ] 安装器部署 Overlay 与全部运行时文件；卸载器清理服务注册但保留用户数据。
